    ...
    <main className="min-h-screen bg-gray-50">
      <header className="bg-green-600 shadow-md">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          <Link
            href="/"
            className="text-white hover:text-green-200 transition-colors text-sm font-medium flex items-center gap-1"
          >
            ← Retour
          </Link>
        ...
        <div className="flex justify-between mt-6">
          {pokemon.id > 1 && (
            <Link
              ...
              className="text-sm text-gray-500 hover:text-green-600 transition-colors capitalize"
            >
          ...
          {pokemon.id < 151 && (
            <Link
              href={`/pokemon/${pokemon.id + 1}`}
              className="text-sm text-gray-500 hover:text-green-600 transition-colors capitalize ml-auto"
            >
              #{String(pokemon.id + 1).padStart(3, "0")} →
            </Link>
          )}
